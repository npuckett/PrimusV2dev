#!/usr/bin/env python3
"""
led_controller.py — PrimusV2 Clean Art-Net LED Controller
...
class ControllerState:
    ...
    def get_json(self):
        with self.lock:
            out = {"fps": self.fps, "output_types": OUTPUT_TYPES, "devices": []}
        ...
        with self.lock:
            ...
            if di is not None and 0 <= di < len(self.devices):
                ...
                if oi is not None and 0 <= oi < len(dev["outputs"]):
                    o = dev["outputs"][oi]
                    if "effect" in data:
                        o["effect"] = str(data["effect"])
                        o["led_state"] = []
                    if "playback" in data:
                        o["playback"] = str(data["playback"])
                        o["led_state"] = []
                    if "speed" in data:
                        o["speed"] = max(0.1, min(10.0, float(data["speed"])))
                    if "angle" in data:
                        o["angle"] = float(data["angle"])
                    if "start_color" in data:
                        o["start_color"] = [int(v) for v in data["start_color"]]
                    if "end_color" in data:
                        o["end_color"] = [int(v) for v in data["end_color"]]
        ...
        with self.lock:
            ...
            for dev in self.devices:
                ...
                for o in dev["outputs"]:
                    speed = o["speed"]
                    scaled_t = t * speed
                    af = compute_anim_factor(scaled_t, o["playback"])
                    fn = EFFECTS.get(o["effect"], fx_none)
                    pixels = fn(
                        count=o["count"], t=scaled_t, dt=dt,
                        speed=speed, anim_factor=af,
                        playback=o["playback"],
                        start_color=tuple(o["start_color"]),
                        end_color=tuple(o["end_color"]),
                        state=o["led_state"],
                        grid=o.get("grid"), angle=o["angle"],
                    )
                    o["pixels"] = [list(p) for p in pixels]
...
.rx-fps{font-size:12px;color:#818cf8;margin-left:8px;font-weight:600}
...
.scan-bar{display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap}
...
<div class="scan-bar">
  <button onclick="doScan()">Scan Network</button>
...
function buildUI() {
  ...
  S.devices.forEach(function(dev, di) {
    ...
    var html = '<div class="dev-header"><h2>' + esc(dev.name) + '</h2>'
      ...
      + '<div class="outputs" id="outputs-' + di + '"></div>';
    div.innerHTML = html;
    app.appendChild(div);

    var oc = div.querySelector("#outputs-" + di);
    dev.outputs.forEach(function(out, oi) {
      var p = document.createElement("div");
      p.className = "output-panel";
      var gl = out.grid ? out.grid[0] + "x" + out.grid[1] : out.count + "px";
      var typeLbl = out.type.replace("_", " ");
      ...
      if (out.grid) {
        angleCtrl = '<div class="ctrl">'
          + '<label>Angle: <span class="val" id="av-'+di+'-'+oi+'">' + Math.round(out.angle) + '&deg;</span></label>'
          + '<input type="range" min="0" max="360" value="' + out.angle + '"'
          + ' oninput="document.getElementById(\'av-'+di+'-'+oi+'\').innerHTML=Math.round(this.value)+\'&deg;\';'
          + 'apiPost(\'/api/update\',{device:'+di+',output:'+oi+',angle:+this.value})">'
          + '</div>';
      }
      ...
      p.innerHTML = '<h3>' + esc(out.name) + ' &middot; ' + typeLbl + ' &middot; ' + gl + ' &middot; U' + out.universe + '</h3>'
        + '<div class="color-row">'
        + '<div class="color-box"><label>Start</label>'
        + '<input type="color" value="' + sh + '"'
        + ' oninput="apiPost(\'/api/update\',{device:'+di+',output:'+oi+',start_color:hexRgb(this.value)})">'
        + '</div>'
        + '<div class="color-box"><label>End</label>'
        + '<input type="color" value="' + eh + '"'
        + ' oninput="apiPost(\'/api/update\',{device:'+di+',output:'+oi+',end_color:hexRgb(this.value)})">'
        + '</div></div>'
        + '<div class="ctrl"><label>Effect</label>'
        + '<select onchange="apiPost(\'/api/update\',{device:'+di+',output:'+oi+',effect:this.value})">'
        + effectOpts + '</select></div>'
        + '<div class="ctrl">'
        + '<label>Speed: <span class="val" id="sv-'+di+'-'+oi+'">' + out.speed.toFixed(1) + '</span></label>'
        + '<input type="range" min="0.1" max="10" step="0.1" value="' + out.speed + '"'
        + ' oninput="document.getElementById(\'sv-'+di+'-'+oi+'\').textContent=parseFloat(this.value).toFixed(1);'
        + 'apiPost(\'/api/update\',{device:'+di+',output:'+oi+',speed:+this.value})">'
        + '</div>'
        + '<div class="ctrl"><label>Playback</label>'
        + '<select onchange="apiPost(\'/api/update\',{device:'+di+',output:'+oi+',playback:this.value})">'
        + playOpts + '</select></div>'
        + angleCtrl;
      oc.appendChild(p);
    });
...
function sizeCanvases() {
  S.devices.forEach(function(dev, di) {
    ...
    var y = 8;
    dev.outputs.forEach(function(out) {
      y += 16;
      if (out.grid) {
        y += out.grid[1] * 20 + 8;
      } else {
...
function drawPreview() {
  ...
  S.devices.forEach(function(dev, di) {
    ...
    dev.outputs.forEach(function(out) {
      ...
      ctx.fillText(out.name + " (" + out.count + "px) U" + out.universe, 10, y + 10);
      y += 16;
...
def main():
    parser = argparse.ArgumentParser(
        description="PrimusV2 Clean LED Controller — Art-Net only")
    ...
    print("PrimusV2 Clean LED Controller (Art-Net)")
