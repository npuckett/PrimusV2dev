#!/usr/bin/env bash
# upload.sh — Compile & upload primusV2_receiver to ESP32-S3 Reverse TFT Feather
# Usage:
...
SKETCH_DIR="$(cd "$(dirname "$0")/primusV2_receiver" && pwd)"
...
for p in ports:
    ...
    label = addr.get('label', '') if isinstance(addr, dict) else ''
    proto = addr.get('protocol', '') if isinstance(addr, dict) else ''
    boards = p.get('matching_boards', [])
    # Prefer a port that matches our FQBN
    for b in boards:
